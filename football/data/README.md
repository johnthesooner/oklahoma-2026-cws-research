...
## epa_football.csv (5 rows, 2021-2025) — generated by `ingest_pbp.py`
Oklahoma's offensive and defensive EPA per play and success rate with national ranks, from the
sportsdataverse ESPN play-by-play release assets (no API key required). `_cg` columns repeat every
figure with garbage time removed using Connelly's published thresholds (lead > 43/37/27/21 by
quarter). Teams under 300 plays are excluded from ranking. **Not opponent-adjusted.**

## Metrics that are NOT AVAILABLE in this build (not estimated)
Turnover margin, penalties, third-down %, red-zone %, rushing/passing splits, havoc rate, opponent
SRS per game, and pre-2005 adjusted ratings — these need the CollegeFootballData API or scraped box
scores. **Corrected 2026-09-08:** EPA per play, success rate and play-by-play win probability were
previously listed here in error. They are freely available without a key and now live in
`epa_football.csv` (contradictions log C32).
